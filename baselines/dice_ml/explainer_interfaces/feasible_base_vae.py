...
class FeasibleBaseVAE(ExplainerBase):
    ...
    def __init__(self, data_interface, model_interface, **kwargs):
        ...
        self.save_path = self.base_model_dir + self.data_interface.data_name + \
            ...
            '-epoch-' + str(self.epochs) + '-batchsize-' + str(self.batch_size) + \
            '-lr-' + str(self.learning_rate) + '-encoded-' + str(self.encoded_size) + '-' + 'base-gen' + '.pth'
    ...
    def compute_loss(self, model_out, x, target_label):
        ...
        # Validity
        # Looks like this needs the logits and not the predictions. 
        # import ipdb; ipdb.set_trace()
        temp_logits = self.pred_model(x_pred)
        if "Log" in str(temp_logits.grad_fn):
            temp_logits = torch.exp(temp_logits)
        ...
        validity_loss += F.hinge_embedding_loss(
            torch.sigmoid(temp_1[:, 1]) - torch.sigmoid(temp_1[:, 0]), torch.tensor(-1), self.margin, reduction='mean')
        validity_loss += F.hinge_embedding_loss(
            torch.sigmoid(temp_0[:, 0]) - torch.sigmoid(temp_0[:, 1]), torch.tensor(-1), self.margin, reduction='mean')
    ...
    def train(self, pre_trained=False):
        ...
        for epoch in range(self.epochs):
            ...
            ret = loss/batch_num
            print('Train Avg Loss: ', ret, train_size, "epoch: ", epoch)
            if ret.item() < best_loss:
                # print(ret.item(), best_loss)
                best_loss = ret.item()
                torch.save(self.cf_vae.state_dict(), self.save_path)
                print("Saving lowest loss model", best_loss)
    ...
    def generate_counterfactuals(self, query_instance_, total_CFs, desired_class="opposite"):

        # Loading the latest trained CFVAE model
        self.cf_vae.load_state_dict(torch.load(self.save_path))
        self.cf_vae.eval()
        # print("Before: ", query_instance_)
        query_instance = self.data_interface.get_ohe_min_max_normalized_data(query_instance_)
        # print("After: ", query_instance)
        ...
        for i in range(len(query_instance)):
            ...
            for cf_count in range(total_CFs):
                ...
                while cf_label == train_y and attempts < max_attempts:
                    recon_err, kl_err, x_true, x_pred, cf_label = \
                        self.cf_vae.compute_elbo(train_x, 1.0-train_y, self.pred_model)
                    attempts += 1
                    # print(cf_label, train_y, attempts)
        ...
        normalized_cf = self.data_interface.get_ohe_min_max_normalized_data(final_cfs_df)
        normalized_cf = torch.tensor(normalized_cf.to_numpy()).float()
        cf_label = torch.argmax(self.pred_model(normalized_cf), dim=1)
        # This is check for the renormalized after inverse normalization. This is required because DiCE-VAE generates weird numbers for all features which needs to be mapped to actual numbers, but after re-classification some of them may not be a CFE. 
        if cf_label == train_y:
            return x_pred, False
        ...
        return exp.CounterfactualExamples(data_interface=self.data_interface,
                                          ...
                                          desired_class=desired_class), True
