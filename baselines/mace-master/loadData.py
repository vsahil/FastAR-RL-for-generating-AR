...
class Dataset(object):
  ...
  def getTrainTestSplit(self, preprocessing = None, with_meta = False):
    ...
    if with_meta:
      ...
      my_data_frame = copy.deepcopy(self.data_frame_kurz)
      if self.dataset_name == "adult_our":
        all_data = my_data_frame.loc[:,input_cols]
        all_true_labels = my_data_frame.loc[:,output_col]
        adult_random = 50
        X_train, X_test, y_train, y_test = train_test_split(all_data, all_true_labels, 
        test_size=0.20, random_state=adult_random)
      elif self.dataset_name == "german_our":
        all_data = my_data_frame.loc[:,input_cols]
        all_true_labels = my_data_frame.loc[:,output_col]
        german_random = 26
        X_train, X_test, y_train, y_test = train_test_split(all_data, all_true_labels, 
        test_size=0.20, random_state=german_random)
      else:
...
def loadDataset(dataset_name, return_one_hot, load_from_cache = False, debug_flag = True, meta_param = None):
  ...
  def getInputOutputColumns(data_frame):
    ...
    if "label" in all_data_frame_cols or "Label" in all_data_frame_cols:
    ...
    elif "target" in all_data_frame_cols:
      input_cols = [x for x in all_data_frame_cols if 'target' not in x.lower()]
      output_cols = [x for x in all_data_frame_cols if 'target' in x.lower()]
    
    assert len(output_cols) == 1, f'{all_data_frame_cols}'
  ...
  if dataset_name == 'adult':
    ...
    for col_idx, col_name in enumerate(input_cols):
      ...
      if col_name == 'sex':
        ...
        attr_type = 'categorical'   # not binary in our case. 
        # attr_type = 'numeric-int'   # not binary in our case. 
        actionability = 'any' # 'none'
        mutability = True
        data_frame_non_hot[col_name] += 1
        increased_cols.append(col_name)
      ...
      # print(col_name, "see", data_frame_non_hot[col_name].min(), data_frame_non_hot[col_name].max())
    ...
    assert len(attributes_non_hot) == len(data_frame_non_hot.columns)   # This includes the "target"/"label" column


  elif dataset_name == 'german_our':
    file1 = f"{os.path.dirname(os.path.realpath(__file__))}/../../datasets/german_redone.csv"
    dataset = pd.read_csv(file1)
    dataset = dataset[['target', 'Checking-account', 'Months', 'Credit-history', 'Purpose',
       'Credit-amount', 'Savings-account', 'Present-employment-since',
       'Insatllment-rate', 'Personal-status', 'Other-debtors',
       'Present-residence-since', 'Property', 'age', 'Other-installment-plans',
       'Housing', 'Number-of-existing-credits', 'Job',
       'Number-of-people-being-lible', 'Telephone', 'Foreign-worker']]
    ...
    for col_idx, col_name in enumerate(input_cols):
      attr_type = 'numeric-int'
      # if col_idx in numerical_features:
      #   attr_type = 'numeric-real'
      actionability = 'any' # 'none'
      mutability = True
