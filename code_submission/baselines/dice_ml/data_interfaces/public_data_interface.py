...
class PublicData:
    ...
    def normalize_data(self, df):
        ...
        if isinstance(df, pd.DataFrame) or isinstance(df, dict):
            for feature_name in self.continuous_feature_names:
                ...
                value = (df[feature_name] - min_value) / (max_value - min_value)
                result[feature_name] = value  # scales between 0 and 1. We want between -1 and 1. 
                # result[feature_name] = 2 * value -1 
        else:
    ...
    def get_minx_maxx(self, normalized=True):
        ...
        # if normalized:      # This is my case.
        #     minx = np.array([[-1.0] * len(self.ohe_encoded_feature_names)])
