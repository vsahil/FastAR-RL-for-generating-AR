...
def loadModelForDataset(model_class, dataset_string, scm_class = None, experiment_folder_name = None):
  ...
  if not (dataset_string in {'synthetic', 'mortgage', 'twomoon', 'german', 'german_our', 'credit', 'compass', 'adult', 'adult_our', 'test'}):
  ...
  preprocessing = None
  # if dataset_string == "adult_our":
  #   preprocessing = 'scaler'  # scales between -1 and 1 We can't do this. 
  # else:
    
  dataset_obj = loadData.loadDataset(dataset_string, return_one_hot = one_hot, load_from_cache = False, meta_param = scm_class)
  X_train, X_test, y_train, y_test = dataset_obj.getTrainTestSplit(preprocessing)
  X_all = pd.concat([X_train, X_test], axis = 0)
  y_all = pd.concat([y_train, y_test], axis = 0)
  # assert sum(y_all) / len(y_all) == 0.5, 'Expected class balance should be 50/50%.'
  feature_names = dataset_obj.getInputAttributeNames('kurz') # easier to read (nothing to do with one-hot vs non-hot!)
  ...
  if model_class == 'tree':
    ...
    # if dataset_string == "adult_our":
      ...
      # model_our, dataset_our, scaler_our, X_test_our, X_train_our, y_train_our, y_test_our = classifier.train_model_adult(file=file1, parameter=2)
      # return model_our, scaler_our, X_test_our, y_test_our
      # X_train_norm = scaler_our.transform(X_train_our)
      # X_test_norm = scaler_our.transform(X_test_our)
      # assert np.allclose(X_train.to_numpy(), X_train_norm)
      # assert (y_train == y_train_our).all()
      # assert np.allclose(X_test.to_numpy(), X_test_norm)
      # assert (y_test == y_test_our).all()

      # adult_random = 50
      # # Very low accuracy, can't use. 
      # model_pretrain = MLPClassifier(solver='lbfgs', alpha=1e-5,
      #                     hidden_layer_sizes=(5, 3), max_iter=10000, random_state=adult_random)
    # else:
  ...
  print('[INFO] done.\n', file=log_file)
  print('[INFO] done.\n')
  assert accuracy_score(y_train, model_trained.predict(X_train)) > 0.69
