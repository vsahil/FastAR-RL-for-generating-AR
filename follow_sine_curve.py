...
class environment:
    ...
    def step(self, action):
        ...
        term1 = self.model(self.current_state)
        term2 = self.model(self.previous_state)
        term3 = self.dist_lambda * abs(nearest_dist)
        # print(round(term1, 2), round(term2, 2), round(term3, 2), "hello")
        return self.current_state, -term3, done, nearest_points
    ...
    def model(self, pt):
        ...
        # return 0
        return x1/5        # [0, 1], there should not be an absolute value
    ...
    def prediction(self, pt):
        ...
        try:
            ...
            # if pt.shape == (1, 2):
            x1, x2 = pt[0]
        if x1 >= 9.8:      # changed from 5 to this, full curve tracing
            return True
        return False
...
def plot_trajectories(x, success_rate, deter=False, closest_points=None, dist_lambda=None):
    ...
    if closest_points:      # is not None
        if deter:
            plt.savefig(f'plots/follow_sine/trajectories_knn_l2_deter_{closest_points}_{dist_lambda}.png')
        else:
            plt.savefig(f'plots/follow_sine/trajectories_knn_l2_nondeter_{closest_points}_{dist_lambda}.png')
    else:
        if deter:
            plt.savefig(f'plots/follow_sine/trajectories_noknn_deter.png')
        else:
            plt.savefig(f'plots/follow_sine/trajectories_noknn_nondeter.png')
    print("PLOT DONE")
...
def use_policy(policy, env, file, deter=False, closest_points=None, dist_lambda=None):
    ...
    def return_counterfactual(original_individual, transit):
        ...
        while attempt_no < maxtry:
            ...
            if env.prediction(new_pt.reshape(1, -1)):       # if this is equal to 1
                ...
                # if (new_pt == individual):        # we don't this part now
                #     print("unsuccessful: ", original_individual)
                #     return transit+1, cost, None, None
                individual = new_pt
        else:
            transit += 1
            print(original_individual, f"successful: {new_pt}",  cost)
            return transit, cost, env.distance_to_closest_k_points(new_pt), path
    ...
    print(successful_transitions, len(undesirable_x), "hello")
    trajectories = random.choices(trajectories, k=3)
    plot_trajectories(trajectories, success_rate, deter, env.no_points, env.dist_lambda)
...
def plot_nearest_point(env, no_points):
    # import ipdb; ipdb.set_trace()
    # pt = X_train.sample().to_numpy()[0]
    plt.figure()
    for _ in range(5):
        x1 = (env.max_x1 - env.min_x1) * np.random.random_sample() + env.min_x1
        x2 = (env.max_x2 - env.min_x2) * np.random.random_sample() + env.min_x2
...
def learn(n_actions, clf, X_train, closest_points, dist_lambda, episodes, gamma, learning_rate, file, deter):
    ...
    # plot_nearest_point(env, no_points=closest_points)
    policy = Policy(env, gamma)
    optimizer = optim.Adam(policy.parameters(), lr=learning_rate)
    final_policy = main(episodes, env, policy, optimizer)
    # print(final_policy, "Done")
    # import ipdb; ipdb.set_trace()
    use_policy(final_policy, env, file, deter, closest_points, dist_lambda)
...
if experiment:
    for closest_points in [1, 2, 5, 10]:
        for dist_lambda in [0.1, 1, 10, 100, 1000, 10000]:
