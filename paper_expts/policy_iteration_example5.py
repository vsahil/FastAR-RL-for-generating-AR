...
class environment:
	def __init__(self, n_states, n_actions, clf, discount_factor, scaler, dataset, lambda_):	#, X_train=None, closest_points=None, dist_lambda=None):
		self.nS = n_states
		self.nA = n_actions
		self.classifier = clf
		self.states = {}
		...
		for sts in list(itertools.product(*x)):
			for act in range(self.nA):
				...
				if self.dataset.iloc[:, feature_changing].name in self.immutable_features:
					self.P[sts_sequence][act] = [(1.0, sts_sequence, -10, None)]	# never change immutable feature
					continue
				# age can't decrease
				...
				if decrease:
					if sts[feature_changing] > values[0]:
						...
						reward = self.model(next_state) - 1	- knn_dist_loss	# constant cost for each action		# - self.dist_lambda * self.distance_to_closest_k_points(a+1, b, c)
					...
					if sts[feature_changing] < values[-1]:
						next_state_ = self.states[tuple(next_state)]
						reward = self.model(next_state) - 1 - knn_dist_loss
						ret = [(1.0, next_state_, reward, None)]
					else:
...
def policy_eval(policy, theta=1e-8, max_iterations=1000):
...
def policy_improvement(policy_eval_fn=policy_eval):
	...
	while True:
		...
		for s in range(env.nS):	# this loop also requires about 38s, down to 1.3s
			...
			diff_ = abs(action_values[best_a] - action_values[chosen_a])
			# if diff_ < 1e-6:
			# 	policy[s] = np.eye(env.nA)[best_a]
			# 	continue
		...
		# if len(unstable_list) < 50:
		diff_max = 0
		if len(unstable_list) > 0:
			diff_max = max([i[1] for i in unstable_list])
		print(unstable_list, diff_max)
		if diff_max < 1e-6:
			policy_stable =True
		epoch += 1
...
if __name__ == "__main__":
	...
	if experiment:
		...
		print("See lambda_: ", lambda_)
		env = environment(n_states, n_actions, clf, gamma, scaler, dataset, lambda_) #, X_train=X_train, closest_points=closest_points, dist_lambda=dist_lambda)
		# final_policy, V = policy_improvement()
		# np.save(f'final_policy_example{example}_{lambda_}.npy', final_policy) # save
		# np.save(f'value_example{example}_{lambda_}.npy', V) # save
		final_policy = np.load(f'saved_policies/final_policy_example{example}_{lambda_}.npy')
		V = np.load(f'saved_policies/value_example{example}_{lambda_}.npy')
	...
	with open("expt_results.csv", "a") as f:
		print(f"Example{example}, {percentage_success}, {avg_path_len}, {avg_knn_dist}, {time_taken}, {lambda_}", file=f)
	print("DONE")


# [724, 725, 726, 727, 728, 730, 732, 733, 734, 735, 756, 757, 758, 759, 760, 762, 764, 765, 766, 767, 1236, 1237, 1238, 1239, 1240, 1242, 1244, 1245, 1246, 1247, 1268, 1269, 1270, 1271, 1272, 1274, 1276, 1277, 1278, 1279]
# [724, 725, 726, 727, 728, 730, 732, 733, 734, 735, 756, 757, 758, 759, 760, 762, 764, 765, 766, 767, 1236, 1237, 1238, 1239, 1240, 1242, 1244, 1245, 1246, 1247, 1268, 1269, 1270, 1271, 1272, 1274, 1276, 1277, 1278, 1279]
# 
# 